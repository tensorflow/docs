...
[`class FixedLenSequenceFeature`](./tf/FixedLenSequenceFeature): Configuration for a dense input feature in a sequence item.
...
[`class SparseFeature`](./tf/SparseFeature): Configuration for parsing a sparse input feature.
...
[`fake_quant_with_min_max_vars(...)`](./tf/fake_quant_with_min_max_vars): Fake-quantize the 'inputs' tensor of type float and shape `[b, h, w, d]` via
...
[`image`](./tf/image) module: Image processing and decoding ops. See the [Images](../../api_guides/python/image) guide.

[`import_graph_def(...)`](./tf/import_graph_def): Imports the TensorFlow graph in `graph_def` into the Python `Graph`.
...
[`matching_files(...)`](./tf/matching_files): Returns the set of files matching a pattern.
...
[`nn`](./tf/nn) module: ## Neural network support. See the [Neural Network](../../api_guides/python/nn) guide.
...
[`pywrap_tensorflow`](./tf/pywrap_tensorflow) module
...
[`test`](./tf/test) module: Testing. See the [Testing](../../api_guides/python/test) guide.
...
[`train`](./tf/train) module: Support for training models.  See the [Training](../../api_guides/python/train) guide.

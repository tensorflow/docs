...
[`assert_rank_at_least(...)`](../tf/debugging/assert_rank_at_least): Assert that `x` has rank of at least `rank`.

[`assert_rank_in(...)`](../tf/debugging/assert_rank_in): Assert that `x` has a rank in `ranks`.
...
[`assert_type(...)`](../tf/debugging/assert_type): Asserts that the given `Tensor` is of the specified type.

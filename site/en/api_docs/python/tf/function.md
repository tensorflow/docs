...
The `call` methods of a <a href="../tf/keras/Model"><code>tf.keras.Model</code></a> subclass can be decorated with
...
Calling a <a href="../tf/function"><code>tf.function</code></a> with tensor arguments of different dtypes should lead
to at least one computational graph per distinct set of dtypes. Alternatively,
always calling a <a href="../tf/function"><code>tf.function</code></a> with tensor arguments of the same shapes and
...
To control the tracing behavior, use the following tools:
 - different <a href="../tf/function"><code>tf.function</code></a> objects are guaranteed to not share traces; and
...
* <b>`func`</b>: function to be compiled. If `func` is None, returns a decorator that
  ...
  <a href="../tf/function"><code>tf.function(func, input_signature=...)</code></a>. The former can be used to

...
# tf.contrib.bayesflow.layers.DenseVariational
...
* <b>`kernel_use_local_reparameterization`</b>: Python `bool` indicating whether
    `kernel` calculation should employ the Local Reparameterization Trick.
* <b>`kernel`</b>: `VariationalKernelParamater` instance containing all `kernel`
    related properties and `callable`s.
* <b>`bias`</b>: `VariationalParameter` instance containing all `kernel`
    related properties and `callable`s.
...
<h3 id="kernel_use_local_reparameterization"><code>kernel_use_local_reparameterization</code></h3>

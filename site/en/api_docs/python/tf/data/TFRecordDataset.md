...
Defined in [`tensorflow/python/data/ops/readers.py`](https://www.github.com/tensorflow/tensorflow/blob/r1.7/tensorflow/python/data/ops/readers.py).
...
* <b>`filenames`</b>: A `tf.string` tensor or `tf.data.Dataset` containing one or
    more filenames.
...
* <b>`num_parallel_reads`</b>: (Optional.) A `tf.int64` scalar representing the

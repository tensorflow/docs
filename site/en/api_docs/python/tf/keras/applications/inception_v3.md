

page_type: reference
<style> table img { max-width: 100%; } </style>


<!-- DO NOT EDIT! Automatically generated file. -->


# Module: tf.keras.applications.inception_v3



Defined in [`tensorflow/python/keras/applications/inception_v3/__init__.py`](https://www.github.com/tensorflow/tensorflow/blob/r1.7/tensorflow/python/keras/applications/inception_v3/__init__.py).

Inception V3 Keras application.

## Functions

[`InceptionV3(...)`](../../../tf/keras/applications/InceptionV3): Instantiates the Inception v3 architecture.

[`decode_predictions(...)`](../../../tf/keras/applications/densenet/decode_predictions): Decodes the prediction of an ImageNet model.

[`preprocess_input(...)`](../../../tf/keras/applications/inception_v3/preprocess_input): Preprocesses a numpy array encoding a batch of images.


...
* <b>`width_shift_range`</b>: fraction of total width, if < 1, or pixels if >= 1.
* <b>`height_shift_range`</b>: fraction of total height, if < 1, or pixels if >= 1.
* <b>`brightness_range`</b>: the range of brightness to apply
* <b>`shear_range`</b>: shear intensity (shear angle in degrees).
* <b>`zoom_range`</b>: amount of zoom. if scalar z, zoom will be randomly picked
        in the range [1-z, 1+z]. A sequence of two can be passed instead
        to select this range.
* <b>`channel_shift_range`</b>: shift range for each channel.
...
* <b>`validation_split`</b>: fraction of images reserved for validation (strictly
      between 0 and 1).

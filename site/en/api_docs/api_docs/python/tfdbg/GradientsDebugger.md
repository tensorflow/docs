...
differentiation algorithm, i.e., [`tf.gradients`](../tf/gradients) and optimizer classes that
use it.
...
* <b>`input_tensor`</b>: the input `tf.Tensor` object whose related gradient tensors
    ...
    are created, e.g., during [`tf.gradients`](../tf/gradients) calls or the construction
    of optimization (training) op that uses [`tf.gradients`](../tf/gradients).

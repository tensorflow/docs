...
[`tf.nn.sampled_softmax_loss`](../../../tf/nn/sampled_softmax_loss). For a description of the algorithm and some
...
* In the first phase, `num_sampled` classes are selected using
  [`tf.nn.learned_unigram_candidate_sampler`](../../../tf/nn/learned_unigram_candidate_sampler) or supplied `sampled_values`.
  The logits are calculated on those sampled classes. This phases is
  similar to [`tf.nn.sampled_softmax_loss`](../../../tf/nn/sampled_softmax_loss).
...
See [`tf.nn.sampled_softmax_loss`](../../../tf/nn/sampled_softmax_loss) for more documentation on sampling and
...
* <b>`partition_strategy`</b>: A string specifying the partitioning strategy, relevant
      ...
      See [`tf.nn.embedding_lookup`](../../../tf/nn/embedding_lookup) for more details.

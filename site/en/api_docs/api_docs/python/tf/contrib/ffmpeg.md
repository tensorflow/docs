

page_type: reference


<!-- DO NOT EDIT! Automatically generated file. -->


# Module: tf.contrib.ffmpeg



Defined in [`tensorflow/contrib/ffmpeg/__init__.py`](https://www.github.com/tensorflow/tensorflow/blob/r1.4/tensorflow/contrib/ffmpeg/__init__.py).

Working with audio using FFmpeg.

See the [FFmpeg (contrib)](../../../../api_guides/python/contrib.ffmpeg) guide.


## Functions

[`decode_audio(...)`](../../tf/contrib/ffmpeg/decode_audio): Create an op that decodes the contents of an audio file.

[`encode_audio(...)`](../../tf/contrib/ffmpeg/encode_audio): Creates an op that encodes an audio file using sampled audio from a tensor.


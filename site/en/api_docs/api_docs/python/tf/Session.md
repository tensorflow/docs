...
* <b>`accept_options`</b>: (Optional.) Iff `True`, the returned `Callable` will be
    able to accept [`tf.RunOptions`](../tf/RunOptions) and [`tf.RunMetadata`](../tf/RunMetadata) as optional
    keyword arguments `options` and `run_metadata`, respectively, with
    the same syntax and semantics as [`tf.Session.run`](../tf/Session#run), which is useful
